...
c,a)};n.nice=function(c){return n.domain(tc(f,wf(hb(f,c)[2])))};n.exponent=function(e){if(!arguments.length)return a;g=vc(a=e);b=vc(1/a);c.domain(f.map(g));return n};n.copy=function(){return Af(c.copy(),a,f)};return Cd(n,c)}function vc(c){return function(a){return 0>a?-Math.pow(-a,c):Math.pow(a,c)}}function Bf(c,a){function f(f){return b[((g.get(f)||("range"===a.t?g.set(f,c.push(f)):NaN))-1)%b.length]}function n(a,r){return w.range(c.length).map(function(c){return a+r*c})}var g,b,e;f.domain=function(n){if(!arguments.length)return c;
c=[];g=new t;for(var b=-1,l=n.length,e;++b<l;)g.has(e=n[b])||g.set(e,c.push(e));return f[a.t].apply(f,a.a)};f.range=function(c){if(!arguments.length)return b;b=c;e=0;a={t:"range",a:arguments};return f};f.rangePoints=function(g,p){2>arguments.length&&(p=0);var h=g[0],k=g[1],m=(k-h)/(Math.max(1,c.length-1)+p);b=n(2>c.length?(h+k)/2:h+m*p/2,m);e=0;a={t:"rangePoints",a:arguments};return f};f.rangeBands=function(g,p,h){2>arguments.length&&(p=0);3>arguments.length&&(h=p);var k=g[1]<g[0],m=g[k-0],v=(g[1-
k]-m)/(c.length-p+2*h);b=n(m+v*h,v);k&&b.reverse();e=v*(1-p);a={t:"rangeBands",a:arguments};return f};f.rangeRoundBands=function(g,p,h){2>arguments.length&&(p=0);3>arguments.length&&(h=p);var k=g[1]<g[0],m=g[k-0],v=g[1-k],s=Math.floor((v-m)/(c.length-p+2*h));b=n(m+Math.round((v-m-(c.length-p)*s)/2),s);k&&b.reverse();e=Math.round(s*(1-p));a={t:"rangeRoundBands",a:arguments};return f};f.rangeBand=function(){return e};f.rangeExtent=function(){return Cb(a.a[0])};f.copy=function(){return Bf(c,a)};return f.domain(c)}
function Cf(c,a){function f(){var f=0,b=a.length;for(g=[];++f<b;)g[f-1]=w.quantile(c,f/b);return n}function n(c){if(!isNaN(c=+c))return a[w.bisect(g,c)]}var g;n.domain=function(a){if(!arguments.length)return c;c=a.filter(u).sort(I);return f()};n.range=function(c){if(!arguments.length)return a;a=c;return f()};n.quantiles=function(){return g};n.invertExtent=function(f){f=a.indexOf(f);return 0>f?[NaN,NaN]:[0<f?g[f-1]:c[0],f<g.length?g[f]:c[c.length-1]]};n.copy=function(){return Cf(c,a)};return f()}function Df(c,
...
c}}function xc(c,a,f,n){var b=c.__transition__||(c.__transition__={active:0,count:0}),l=b[f];if(!l){var e=n.time,l=b[f]={tween:new t,time:e,ease:n.ease,delay:n.delay,duration:n.duration};++b.count;w.timer(function(n){function p(n){if(b.active>f)return k();b.active=f;l.event&&l.event.start.call(c,m,a);l.tween.forEach(function(f,n){(n=n.call(c,m,a))&&y.push(n)});w.timer(function(){t.c=h(n||1)?wb:h;return 1},0,e)}function h(n){if(b.active!==f)return k();n/=q;for(var e=v(n),x=y.length;0<x;)y[--x].call(c,
...
a){var f=-1,n=c.length,b,l;if(1===arguments.length){for(;++f<n&&!(null!=(b=c[f])&&b<=b);)b=void 0;for(;++f<n;)null!=(l=c[f])&&l>b&&(b=l)}else{for(;++f<n&&!(null!=(b=a.call(c,c[f],f))&&b<=b);)b=void 0;for(;++f<n;)null!=(l=a.call(c,c[f],f))&&l>b&&(b=l)}return b};w.extent=function(c,a){var f=-1,b=c.length,g,l,e;if(1===arguments.length){for(;++f<b&&!(null!=(g=e=c[f])&&g<=g);)g=e=void 0;for(;++f<b;)null!=(l=c[f])&&(g>l&&(g=l),e<l&&(e=l))}else{for(;++f<b&&!(null!=(g=e=a.call(c,c[f],f))&&g<=g);)g=void 0;
...
point:z,lineStart:z,lineEnd:z,polygonStart:function(){lb.reset();Ea.lineStart=Gg},polygonEnd:function(){var c=2*lb;Db+=0>c?4*ia+c:c;Ea.lineStart=Ea.lineEnd=Ea.point=z}};w.geo.bounds=function(){function c(c,a){F.push(J=[m=c,q=c]);a<v&&(v=a);a>s&&(s=a)}function a(f,b){var g=mb([f*ha,b*ha]);if(S){var r=ub(S,g),r=ub([r[1],-r[0],0],r);hc(r);var r=ic(r),n=f-t,l=0<n?1:-1,e=r[0]*sa*l,n=180<ja(n);n^(l*t<e&&e<l*f)?(r=r[1]*sa,r>s&&(s=r)):(e=(e+360)%360-180,n^(l*t<e&&e<l*f))?(r=-r[1]*sa,r<v&&(v=r)):(b<v&&(v=
b),b>s&&(s=b));n?f<t?p(m,f)>p(m,q)&&(q=f):p(f,q)>p(m,q)&&(m=f):q>=m?(f<m&&(m=f),f>q&&(q=f)):f>t?p(m,f)>p(m,q)&&(q=f):p(f,q)>p(m,q)&&(m=f)}else c(f,b);S=g;t=f}function f(){L.point=a}function b(){J[0]=m;J[1]=q;L.point=c;S=null}function g(c,f){if(S){var b=c-t;B+=180<ja(b)?b+(0<b?360:-360):b}else y=c,E=f;Ea.point(c,f);a(c,f)}function l(){Ea.lineStart()}function e(){g(y,E);Ea.lineEnd();ja(B)>Z&&(m=-(q=180));J[0]=m;J[1]=q;S=null}function p(c,a){return 0>(a-=c)?a+360:a}function h(c,a){return c[0]-a[0]}function k(c,
...
a.x=function(b){if(!arguments.length)return l;l=b;return a};a.y=function(b){if(!arguments.length)return e;e=b;return a};a.out=function(b){if(!arguments.length)return g;g=b;return a};return a};var hi=w.map({"inside-out":function(a){var b=a.length,f,n=a.map(uh),g=a.map(vh),l=w.range(b).sort(function(a,c){return n[a]-n[c]}),e=0,p=0,h=[],k=[];for(a=0;a<b;++a)f=l[a],e<p?(e+=g[f],h.push(f)):(p+=g[f],k.push(f));return k.reverse().concat(h)},reverse:function(a){return w.range(a.length).reverse()},"default":vd}),
...
basis:Jf,"basis-open":function(a){if(4>a.length)return Ga(a);for(var b=[],f=-1,e=a.length,g,p=[0],h=[0];3>++f;)g=a[f],p.push(g[0]),h.push(g[1]);b.push(Ja(ib,p)+","+Ja(ib,h));for(--f;++f<e;)g=a[f],p.shift(),p.push(g[0]),h.shift(),h.push(g[1]),Hd(b,p,h);return b.join("")},"basis-closed":function(a){for(var b,f=-1,e=a.length,g=e+4,p,h=[],k=[];4>++f;)p=a[f%e],h.push(p[0]),k.push(p[1]);b=[Ja(ib,h),",",Ja(ib,k)];for(--f;++f<g;)p=a[f%e],h.shift(),h.push(p[0]),k.shift(),k.push(p[1]),Hd(b,h,k);return b.join("")},
...
"start").attr("class","nv-legend-text").attr("dy",".32em").attr("dx","8");K.classed("disabled",function(a){return a.disabled});K.exit().remove();K.select("circle").style("fill",function(a,b){return a.color||e(a,b)}).style("stroke",function(a,b){return a.color||e(a,b)});K.select("text").text(m);if(h){var B=[];K.each(function(a,b){var e=d3.select(this).select("text"),k;try{if(k=e.getComputedTextLength(),0>=k)throw Error();}catch(h){k=u.utils.calcApproxTextWidth(e)}B.push(k+28)});for(var R=0,O=0,Q=[];O<
z&&R<B.length;)Q[R]=B[R],O+=B[R++];for(0===R&&(R=1);O>z&&1<R;){Q=[];R--;for(O=0;O<B.length;O++)B[O]>(Q[O%R]||0)&&(Q[O%R]=B[O]);O=Q.reduce(function(a,b,e,k){return a+b})}for(var M=[],H=z=0;z<R;z++)M[z]=H,H+=Q[z];K.attr("transform",function(a,b){return"translate("+M[b%R]+","+(5+20*Math.floor(b/R))+")"});s?T.attr("transform","translate("+(k-b.right-O)+","+b.top+")"):T.attr("transform","translate(0,"+b.top+")");t=b.top+b.bottom+20*Math.ceil(B.length/R)}else{var P=5,y=5,F=0,L;K.attr("transform",function(a,
...
z&&(k.scale(B).ticks(J/100).tickSize(-U,0),V.select(".nv-x.nv-axis").attr("transform","translate(0,"+R.range()[0]+")"),V.select(".nv-x.nv-axis").transition().call(k));D&&(t.scale(R).ticks(U/36).tickSize(-J,0),V.select(".nv-y.nv-axis").transition().call(t));m.dispatch.on("stateChange",function(b){O=b;P.stateChange(O);a.update()});e.dispatch.on("elementMousemove",function(p){b.clearHighlights();var m,q,v,y=[];F.filter(function(a,b){a.seriesIndex=b;return!a.disabled}).forEach(function(e,k){q=u.interactiveBisect(e.values,
p.pointXValue,a.x());b.highlightPoint(k,q,!0);var h=e.values[q];"undefined"!==typeof h&&("undefined"===typeof m&&(m=h),"undefined"===typeof v&&(v=a.xScale()(a.x()(h,q))),y.push({key:e.key,value:a.y()(h,q),color:s(e,e.seriesIndex)}))});if(2<y.length){var E=a.yScale().invert(p.mouseY),J=.03*Math.abs(a.yScale().domain()[0]-a.yScale().domain()[1]),E=u.nearestValueIndex(y.map(function(a){return a.value}),E,J);null!==E&&(y[E].highlight=!0)}E=k.tickFormat()(a.x()(m,q));e.tooltip.position({left:v+h.left,
